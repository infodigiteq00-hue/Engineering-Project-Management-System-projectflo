...
export const fastAPI = {
  ...
  // Project equipment: get activities (with completion if any)
  async getEquipmentActivities(equipmentId: string) {
    try {
      ...
      const completionsRes = await api.get(`/equipment_activity_completions?activity_id=in.(${activityIds})&select=*,updated_by_user:updated_by(full_name)`);
  ...
  async getStandaloneEquipmentActivities(equipmentId: string) {
    try {
      ...
      if (activities.length === 0) return [];
      const activityIds = activities.map((a: any) => a.id).join(',');
      const completionsRes = await api.get(`/standalone_equipment_activity_completions?activity_id=in.(${activityIds})&select=*,updated_by_user:updated_by(full_name)`);
      const completions: any[] = Array.isArray(completionsRes.data) ? completionsRes.data : [];
      const completionByActivityId = completions.reduce((acc: Record<string, any>, c) => {
        acc[c.activity_id] = c;
        return acc;
  ...
  // Sync approved VDCR record to equipment documents
