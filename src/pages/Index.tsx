        ...
        if (firmId) {
          try {
            ...
            if (firmData) {
              ...
              const logo = firmData.logo_url || null;
              setFirmLogo(logo);
              // Persist logo for instant display on refresh
              if (logo) {
      ...
      if (mainTab === 'equipment') {
        try {
              ...
              .then((equipment) => {
                if (equipment && Array.isArray(equipment)) {
                  ...
                  setCache(cacheKey, lightweight, {
                    ttl: 24 * 60 * 60 * 1000, // 24 hours TTL (much longer for standalone)
          ...
          if (standaloneEquipment.length > 0) {
            ...
            const limitedExpired = expiredCache.slice(0, 24);
            setStandaloneEquipment(limitedExpired);
            setStandaloneEquipmentLoading(false);
          ...
          if (equipment && Array.isArray(equipment) && equipment.length > 0) {
            ...
            // FIX: Don't clear state if fetch returns empty - preserve existing state
            // Only clear if we truly have no data and no existing state
            if (standaloneEquipment.length === 0) {
          ...
          // FIX: Don't clear state on error - preserve existing data
          // Only show error toast, but keep existing equipment visible
          toast({
            title: "Error",
            description: "Failed to refresh standalone equipment. Showing cached data.",
            variant: "destructive",
          });
        } finally {
          setStandaloneEquipmentLoading(false);
